...
class GoogleAddressApiTestCase(TestCase):
    ...
    @override_settings(GOOGLE_ADDRESS={'API_KEY': 'test', 'API_LANGUAGE': 'uk'})
    def test_can_request_with_lang(self):
        """ Assert it's possible to make requests setting a key """
        url = GoogleAddressApi()._get_url()
        self.assertTrue("language=uk" in url)

    @override_settings(GOOGLE_ADDRESS={'API_KEY': 'test', 'API_LANGUAGE': 'uk'})
    def test_can_request_with_lang_in_class_kwargs(self):
        """ Assert it's possible to make requests setting a key """
        url = GoogleAddressApi(language='ru')._get_url()
        self.assertTrue("language=ru" in url)
